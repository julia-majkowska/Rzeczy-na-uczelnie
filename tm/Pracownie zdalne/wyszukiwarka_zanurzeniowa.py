...
#def read_vectors(): 
#    with open('indeksy/document_vectors_matrix.txt', 'r') as f:
#        matrix= f.read()
#        return jsonpickle.decode(matrix)

#vector_matrix = read_vectors()
...
def get_vector(doc_id): 
    cur = conn.cursor()
    cur.execute('''SELECT vect FROM dense_vect where id = \'%s\''''%doc_id)
    lista = cur.fetchall()
    #print(lista)
    cur.close()
    conn.commit()
...
def closest_K_document(query_doc_id):
    ...
    query_toks = word_tokenize(first_part)[:25]
    K = 11
    candidates = list(find_cands(query_toks, K))
    ...
    vector_matrix = np.array([get_vector(i) for i in candidates])
    
    print("Looking for k closest documents")
    knn = NearestNeighbors(n_neighbors = K) #tutaj definiujemy k
    knn.fit(vector_matrix, [K, 100])
    neighbours = knn.kneighbors(v_query.reshape(1, -1), return_distance=False)
    return [candidates[i] for i in neighbours[0]]
