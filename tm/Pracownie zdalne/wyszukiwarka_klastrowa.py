...
def read_clusters(): 
    ...
    with open("indeksy/lemat_to_cluster10.txt", 'r')as f: 
        ...
        while line : 
            ...
            cluster_lemats[lemat.lower()] = num_cluster
            line = f.readline()
    return cluster_lemats
cluster_lemats = read_clusters()
...
def read_lemats() : 
    ...
    with open('polimorfologik-2.1.txt') as f:
        ...
        while l: 
            words = l.split(';')[:2]
            if(words[0].lower() in cluster_lemats.keys()) : 
                res[words[1]].add(("C", cluster_lemats[words[0].lower()]))
            else: 
                res[words[1]].add(("W", words[0]))
            l= f.readline()
    return res
...
def get_indeks(id_listy): 
    ...
    if(id_listy[0] == "W") : 
        ...
        cur.execute('''SELECT postlist FROM cluster_indeks where id = \'%s\''''%str(slowo))
    lista = cur.fetchall()
    #print(lista)
    cur.close()
    conn.commit()
    try: 
        res = jsonpickle.decode(lista[0][0])
        #print(res)
        return res
    except: 
        print(id_listy)
