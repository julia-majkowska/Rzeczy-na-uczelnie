...
void generate_permutation(int *array, int size, int jump) {
...
int main(int argc, char **argv) {
  int opt, size = -1, steps = -1, times = -1, jump = -1;
  bool error = false;

  while ((opt = getopt(argc, argv, "n:s:t:j:")) != -1) {
    if (opt == 'n') 
      size = 1 << atoi(optarg);
    else if (opt == 's')
      steps = 1 << atoi(optarg);
      ...
      times = atoi(optarg);
    else if (opt == 'j')//dodaje parametr j oznaczajacy offset przeskoku pomiedzy elementami permutacji
      jump = atoi(optarg);
    else
      error = true;
  }

  if (error || size < 0 || steps < 0 || times < 0 || jump < 0 ) {
    printf("Usage: %s -n log2(size) -s log2(steps) -t times -j jump offset\n", argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  generate_permutation(array, size, jump);
  flush_cache();
